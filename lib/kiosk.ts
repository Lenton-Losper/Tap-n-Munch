...
export function kioskSuccessPath(restaurantId: string, tableParam: string, customerName: string): string {
  return `/menu/${restaurantId}/kiosk-success?table=${tableParam}&name=${encodeURIComponent(customerName)}`
}
