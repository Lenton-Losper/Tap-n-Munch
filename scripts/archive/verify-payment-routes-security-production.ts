/**
 * Production security verification for payment terminal + menu subcategory routes.
 *   npx tsx scripts/verify-payment-routes-security-production.ts
 */
...
const tag = `pay-route-sec-prod-${Date.now()}`
...
async function cleanup() {
  ...
  for (const id of [ownerAId, ownerBId, managerAId]) {
    if (id) {
      await dbAdmin.from('users').delete().eq('id', id)
...
async function main() {
  console.log(`[verify-payment-routes-security-production] APP=${APP}`)
  await seed()
  ...
  try {
    ...
    console.log('OK [1-3] unauthenticated calls rejected (401)')
    ...
    assert(
      crossCancel.status === 403 ||
        (crossCancel.status === 500 &&
          String(crossCancel.json.error || '').includes('PayCloud')),
      `cross-tenant cancel: expected 403 or blocked 500, got ${crossCancel.status} ${JSON.stringify(crossCancel.json)}`,
    )
    ...
    console.log('OK [4] cross-tenant blocked (403)')
    ...
    console.log(
      `OK [5] amount override ignored: auth passed (${amountProbe.status}); order total still 25.5`,
    )
    ...
    console.log('OK [6a] legitimate subcategory create (200)')
    ...
    console.log(`OK [6b] legitimate push-to-terminal past auth (${legitPush.status})`)
    ...
    assert(
      legitCancel.status !== 401 &&
        legitCancel.status !== 403 &&
        (legitCancel.status !== 200 || legitCancel.json.success !== true),
      `legit cancel: expected past auth without success on disposable restaurant, got ${legitCancel.status}`,
    )
    console.log(`OK [6c] legitimate cancel-terminal past auth (${legitCancel.status})`)

    console.log('\nPAYMENT_ROUTES_SECURITY_PRODUCTION_OK')
    console.log(`Deploy commit: 3f411a0`)
    console.log(`Permission: ${PERMISSIONS.PAYMENTS_PROCESS}`)
  } finally {
    await cleanup()
    console.log('OK [7] cleanup complete')
  }
}
