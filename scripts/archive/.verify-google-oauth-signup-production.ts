/**
 * Production verification for Google OAuth new-restaurant signup (#35),
 * post-migration-sync. Same assertions as scripts/verify-google-oauth-signup-staging.ts,
 * pointed at production, with a throwaway test account cleaned up in finally.
 */
...
const TEST_PASSWORD = randomBytes(24).toString('base64')

if (!SUPABASE_URL.includes(PROD_REF) || !SERVICE_KEY) {
  throw new Error('Refusing: production Supabase credentials missing (.env.production.local)')
}
...
async function main() {
  ...
  const email = `prod-oauth-verify-${tag}@example.com`
  const stalePublicUserId = randomUUID()
  let authUserId: string | null = null
  let restaurantId: string | null = null

  console.log(`[${ts()}] === Google OAuth signup PRODUCTION verification (post-migration-sync) ===`)
  ...
  try {
    ...
    const { data: authData, error: authError } = await admin.auth.admin.createUser({
      ...
      email_confirm: true,
      user_metadata: {
        full_name: `Prod OAuth Verify ${tag}`,
    ...
    const ensured = await ensurePublicUserForOAuth(admin, {
      id: authUserId,
      email,
      fullName: `Prod OAuth Verify ${tag}`,
    ...
    await assert(publicUser?.id !== stalePublicUserId, 'stale public.users id was removed/replaced')
    ...
    restaurantId = await createRestaurantForUserAtomic(admin, {
      ...
      fullName: `Prod OAuth Verify ${tag}`,
      phone: '0812345678',
      restaurantName: `Prod OAuth Verify Restaurant ${tag}`,
    })
    ...
    console.log(`\n[${ts()}] Google OAuth signup PRODUCTION verification passed.`)
  } finally {
