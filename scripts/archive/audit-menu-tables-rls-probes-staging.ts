/**
 * Step 1 live probes: cross-tenant menu table writes via anon-key Supabase client.
 *   npx tsx scripts/audit-menu-tables-rls-probes-staging.ts
 */
...
const tag = `menu-rls-probe-${Date.now()}`
...
async function main() {
  ...
  const report = {
    ...
    ownerSameTenantWrites: {
      ...
      update_subcategory: await probeUpdate(ownerClient, 'menu_subcategories', subcategoryAId!, {
        name: `${tag} own sub renamed`,
      }),
      ...
      update_item: await probeUpdate(ownerClient, 'menu_items', itemAId!, {
        name: `${tag} own item renamed`,
      }),
    },
