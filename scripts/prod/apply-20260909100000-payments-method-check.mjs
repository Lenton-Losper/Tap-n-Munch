/**
 * Apply supabase/migrations/20260909100000_payments_method_check.sql to PRODUCTION over a
 ...
 *   node scripts/prod/apply-20260909100000-payment-intents.mjs            # dry run
 *   node scripts/prod/apply-20260909100000-payment-intents.mjs --confirm  # applies
 */
...
const VERSION = '20260909100000'
const PATH = 'supabase/migrations/20260909100000_payments_method_check.sql'
...
async function main() {
  ...
  console.log('APPLY 20260909100000_payments_method_check TO PRODUCTION — direct Postgres')
  console.log('='.repeat(78))
  ...
  try {
    ...
    const { rows: tbl } = await client.query(
      `SELECT EXISTS (
         SELECT 1 FROM pg_constraint
          WHERE conname = 'payments_method_valid_values'
            AND conrelid = 'public.payments'::regclass
       ) AS present`,
    )
    const tablePresent = tbl[0].present
    const inLedger = applied.has(VERSION)
    console.log(`  constraint present: ${tablePresent ? 'yes' : 'no'}`)
    ...
    console.log(`  definition: ${def[0]?.d ?? '(ABSENT)'}`)

    /**
     * POSITIVE CONTROL AND NEGATIVE CONTROL, both rolled back.
     *
     * "The DDL ran" does not prove a CHECK bites -- a constraint that accepts everything looks the
     * same in pg_constraint as one that works. So an illegal value is attempted (must fail) and a
     * legal one is attempted (must succeed). Without the second, a table that rejects EVERY insert
     * for an unrelated reason would read as a working constraint.
     */
    for (const [label, value, shouldFail] of [
      ['paytoday (illegal today)', 'paytoday', true],
      ['cash (legal)', 'cash', false],
    ]) {
      await client.query('BEGIN')
      let failed = false
      let msg = ''
      try {
        await client.query(
          `INSERT INTO public.payments (restaurant_id, amount, method, status)
           SELECT id, 1, $1, 'completed' FROM public.restaurants LIMIT 1`,
          [value],
        )
      } catch (e) {
        failed = true
        msg = String(e.message).split(String.fromCharCode(10))[0]
      }
      await client.query('ROLLBACK')
      const ok = failed === shouldFail
      console.log(
        `  ${ok ? 'ok  ' : 'FAIL'}  ${label.padEnd(26)} ${failed ? 'rejected' : 'accepted'}${
          failed ? ` (${msg.slice(0, 60)})` : ''
        }`,
      )
    }

    const { rows: n } = await client.query('SELECT count(*)::int AS n FROM public.payments')
    console.log(`  rows unchanged: ${n[0].n}`)
  } finally {
