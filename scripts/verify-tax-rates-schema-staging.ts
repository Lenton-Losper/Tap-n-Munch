/**
 * Staging verification: tax_rates schema (table, constraints, RLS) + menu_items.tax_rate_id.
 *   npx tsx scripts/verify-tax-rates-schema-staging.ts
 */
...
function runQuery(sql: string): Row[] {
  const tmp = join(process.cwd(), 'supabase', '.temp', `_tax_rates_q_${Date.now()}.sql`)
...
`)
  ...
  if (!pass) {
    console.error('TAX_RATES_SCHEMA_STAGING_VERIFY_FAIL')
    process.exitCode = 1
  } else {
    console.log('TAX_RATES_SCHEMA_STAGING_VERIFY_OK')
  }
}
