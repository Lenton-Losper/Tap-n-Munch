-- @env: staging
--
-- Recovered 2026-08-05 from the staging migration ledger (issue #143). Scoped staging-only: the
-- body itself says "Do NOT apply this specific migration to production", and the row it seeds
-- references a restaurant id that exists only on staging. The @env header stops the drift guard
-- reporting it as missing on production (#145).
