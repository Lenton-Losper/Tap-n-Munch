-- Atomic Google OAuth / signup restaurant provisioning (#35).
-- Roles payload is passed from TypeScript (role-permissions.config.json) to avoid drift.
...
BEGIN
  ...
  INSERT INTO public.restaurants (name, phone, currency)
  VALUES (btrim(p_restaurant_name), NULLIF(btrim(COALESCE(p_phone, '')), ''), 'NAD')
