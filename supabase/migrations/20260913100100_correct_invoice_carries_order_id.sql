-- @env: both
--
-- correct_invoice() CARRIES order_id ONTO THE CORRECTION AND THE CREDIT NOTE.
--
-- Companion to 20260913100000, which adds `business_documents.order_id`. Split from it because
-- scripts/check-migration-no-data-write.mjs refuses ALTER TABLE and INSERT in one file; the INSERTs
-- here are inside a FUNCTION BODY and write nothing when this migration runs, but the rule cannot
-- distinguish the two and the strict reading is the safe one.
--
-- ================================================================================================
-- WHAT CHANGES, EXACTLY
-- ================================================================================================
--
-- This is the 20260725200000 definition with FOUR lines added, generated from that file rather than
-- retyped so that everything else is byte-identical:
--
--     the replacement invoice's column list   + order_id
--     the replacement invoice's VALUES list   + v_original.order_id
--     the credit note's column list           + order_id
--     the credit note's VALUES list           + v_original.order_id
--
-- Both new rows inherit the ORIGINAL's link, which is the correct lineage: a correction and its
-- credit note bill for the same order the original did. Nothing else about the function -- its
-- guards, its numbering, its tax maths, its void of the original -- is touched.
--
-- Idempotent: CREATE OR REPLACE. Safe to run before or after the column exists, because the
-- function is not validated against the table until it is called.
