-- @env: both
--
-- FIX TO 20260919090000: only the LEAD order carries the merchant order number.
--
-- FOUND ON STAGING, NOT LOCALLY, and the difference is the point. The Docker test harness builds
-- its own schema from supabase/tests/fixture-schema.sql, which did not carry
-- `orders_paycloud_merchant_order_no_unique` -- a GLOBAL partial unique index that has been on
-- both real databases since 20260502120000. Against the real staging schema the very first smoke
-- scenario, the Riviera N$220 + N$500 = N$720 settlement, died with:
--
--     duplicate key value violates unique constraint "orders_paycloud_merchant_order_no_unique"
--     Key (paycloud_merchant_order_no)=(...REF-1) already exists.
--
-- raised by the claim loop's own UPDATE on the SECOND order. Because the whole settlement is one
-- transaction, the exception rolled back every write: the card would have been charged N$720 and
-- the database would hold no payment at all. That is worse than the defect being closed, and it
-- was reachable on every multi-order tab.
--
-- THE RULE WAS ALREADY WRITTEN DOWN. 20260502120000, which created the index, says so in one line:
-- "table receipts share payment_reference; only the lead row holds paycloud_merchant_order_no".
-- The atomic function simply did not honour it. This is that rule, encoded.
--
-- WHY A SECOND FILE. 20260919090000 is applied on staging. Editing an applied migration leaves the
-- database running a definition no committed file describes, and the drift gate compares versions
-- rather than content, so it would not notice. A forward fix is visible in both places.
--
-- The body below is 20260919090000's, byte-identical apart from `v_ref_free` and the one write.
-- Grants are re-asserted because CREATE OR REPLACE keeps them, and stating them costs nothing.
