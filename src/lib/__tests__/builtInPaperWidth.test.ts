...
describe('paperTypeFromWidthMm', () => {
  ...
  it('falls back to 58mm, not 80mm, for a missing or unrecognised width', () => {
    expect(paperTypeFromWidthMm(null)).toBe(PAPER_TYPE_58MM);
    expect(paperTypeFromWidthMm(undefined)).toBe(PAPER_TYPE_58MM);
    expect(paperTypeFromWidthMm(72)).toBe(PAPER_TYPE_58MM);
  });
});
