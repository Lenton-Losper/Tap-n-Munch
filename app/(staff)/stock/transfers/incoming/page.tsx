...
import { IncomingTransfersPanel } from '@/components/stock/incoming-transfers-panel'
...
export default async function IncomingTransfersPage() {
  ...
  const [incoming, outgoing, canCreate, canReceive, canReceiveTransfers, canViewAllLocations, restaurantName] =
    await Promise.all([
      getIncomingTransfersDetailed(supabase, restaurantId),
  ...
  return (
    <div className="min-h-screen bg-[#F7F6F3]">
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 space-y-6">
        ...
        />
        <IncomingTransfersPanel transfers={incoming} canReceive={canReceiveTransfers} />
      </div>
    </div>
  )
}
