...
export async function POST(
  ...
  try {
    ...
    // Split from the status write on purpose. These two were one statement, so when the status
    // update hit the one-open-tab-per-table unique index (reachable when a guest opened a new
    // tab while this one sat in ready_to_pay) the whole statement failed and the flags were
    // never cleared -- leaving a fully paid tab parked on the ready-to-pay queue for good.
    ...
    // Reopening (rather than closing) is the existing contract for both settle paths; closing
    // is a separate staff action. Now error-checked rather than silently swallowed.
