# HANDOVER â€” 2026-08-16. QR customer redesign, overnight build.
...
## STATE AT START OF RUN (measured, 2026-08-16 ~00:10)
    ...
    production /api/version       3c6eec9   cache-busted â€” EXACTLY origin/main, no drift
    origin/cloudflare-staging     1d63b32   copy(tabs): ship the pending-figure copy
    staging /api/version          1d63b32   cache-busted â€” EXACTLY origin/cloudflare-staging
    main â†” staging divergence     119 commits on main not on staging / 150 the other way
    staging DB migration drift    136 local / 136 applied â€” OK, CLEAN
...
- `docs/qr-flow-audit.md` (4146 lines) â€” on branch `docs/qr-flow-audit`, worktree `wt-qr-audit`,
  at `936790a`. **Not present on `cloudflare-staging` or `main`.**
- `docs/agent-operating-contracts.md` â€” revision 3 plus the 2026-08-15 amendment to section 1
  step 2 (the `.env.local` production-credentials near-miss).
- `docs/handover-2026-08-11-sprint.md` â€” last checkpoint is **2026-08-14**. It does not cover the
...
|---|---|
| `f46ccaa` | **the item popup opens for EVERY item**, including one with no options â€” spec Â§11 |
| `dd28812` | Receipt and My Orders no longer share an icon â€” spec Â§7 |
| `b512c1c` / `4861492` | tab PIN readable by every member, rendered **once**, in the strip â€” spec Â§10 |
| `5b254df` | **one authoritative tab total** (QRA-12 / #119), and honest polling |
| `7d669e0` / `1d63b32` | **two figures everywhere â€” payable and pending** â€” spec Â§26 |
| `66c0118` | `orders.source_request_id` â€” the requestâ†’order link |
| `6f2284c` | QRA-01, the edit lock refusing its own holder |

So spec Â§40.I ("verify whether popup-for-every-item is live, staging-only, or not implemented")
answers: **staging-only.** And spec Â§26's two-figure requirement is largely built already.
...
### Piece 1 â€” item sheet for every item Â· `qrd/1-item-sheet` Â· merged `e2906a1`

The popup-for-every-item half already existed. What was missing: the card itself was not
tappable â€” only the 9Ã—9 round `+` button opened it, on both the list card and the Popular
carousel card.
...
both callers ask it. `isChargeableMenuStatus` deliberately did **not** move â€” it stays inside
...
- **Live on staging:** `e2906a1607c91dc42b491dbd82c34ccb3a91b574`, cache-busted Ã—2.

### Piece 2 â€” Place Order lands on My Orders Â· `qrd/2-my-orders-destination` Â· merged `71fb9e0`

Spec Â§16. The tab path used to clear the cart, raise a toast, and `router.replace` back to
...
placed until staff Accept â€” so the customer landed on a screen showing a number that excluded
...
**Scope deliberately narrowed, and this is a deviation from the spec â€” see DEVIATIONS below.**
...
## ORDER OF WORK â€” CHANGED FROM THE BRIEF, WITH THE REASON

The brief's suggested order was 1 item sheet Â· 2 menu simplification Â· 3 My Orders destination,
and said to adjust if the code says otherwise. **2 and 3 are swapped.** The code says otherwise:

`MenuOrderStatusTracker` is the six-step tracker the spec wants off the menu â€” and it is *also*
...
1. **Piece 2 moves only the TAB path to My Orders. The non-tab path still lands on
   ...
   file and is untouched â€” the spec already says to preserve it technically.

2. **The inline variant chips stay on the menu card for now.**
   Spec Â§11 wants one interaction per item, and chips on the card are arguably a second
...
## OPEN â€” TO BE FILLED AS THE RUN CONTINUES

- Section 40 assessment Aâ€“L: in progress; the terminal (G) and backend (D/E/J) halves are being
  established by dedicated read-only investigations rather than asserted from the audit alone.
- Events Aâ€“Q simulation results.
- PENDING COPY list.
...
| String | Renders |
|---|---|
| `PENDING COPY â€” Order sent to the restaurant` | My Orders, temporary banner after Place Order (6s) |
...
BOM**. It silently turned `PENDING COPY â€” ` into `PENDING COPY Ã¢â‚¬â€ ` in a source file. It was
...
from `.env.test` â€” and check the URL carries the staging ref `mdqjpxwczrhkxkbqatqa` before doing
...
a clean `tsc --noEmit` says **nothing** about that file â€” eslint was the only local gate that
could have caught it. Now run per piece before every push.

**A `**` before a path closes a block comment.** `**/view is deliberatelyâ€¦` inside a docblock ends
...
# CHECKPOINT 2 â€” pieces 3 and 5 shipped. The biggest real gap found and closed.
    ...
    staging DB drift              136/136 CLEAN â€” no piece so far carries a migration

## Piece 3 â€” menu simplification Â· `qrd/3-menu-simplification` Â· merged `cf7c1a4` Â· LIVE
...
**A live defect was found and fixed inside it.** The strip was a three-way ternary â€” one arm JSX,
two template strings. The pending figure was appended to the two template-string arms and **not**
to the JSX arm, which is the arm taken *whenever the tab has a PIN*. So on a PIN-protected tab â€”
the ordinary case â€” a customer who had just ordered saw the payable figure with nothing naming
...
## Piece 5 â€” the shared tab Â· `qrd/5-shared-tab` Â· merged `85a945c`

**THE BIGGEST FINDING OF THE RUN.** `/menu/[id]/tab` already grouped by member and already showed
a server-derived table total â€” and the list it grouped came from `fetchOrdersForTab`, which is
scoped to the ids **this browser** holds. Lenton saw *"Lenton â€” Burger N$95"* under a heading
...
two figures and then throws the rows away â€” adding lines there would have cost nothing.
...
the table ate â€” the tab UUID travels in a `?tabId=` URL that `tab-context` adopts into
...
- `edit_lock_token` is **not SELECTed at all** â€” the surest way not to leak a capability from a
...
- **Never infers a financial relationship.** An order whose member cannot be resolved goes to an
  ...
  group instead â€” that one *is* attributed, to someone who left the array or ordered before
...
## Status vocabulary â€” SIX words, not four. A deliberate spec deviation.
...
| `accepted` / `confirmed` | *merged into "being prepared"* | **Accepted** â€” kept separate |
...
| anything else | *fell through to "ðŸŽ‰ New"* | **unknown** â€” promises nothing |

`accepted` is not merged into "being prepared" because staff taking an order is not the kitchen
starting it â€” **and it is the boundary editing closes at**, so the two states differ in what the
customer can still *do*, not only in what they are told.

**The fallback is the other half of that module.** `my-orders` ended
`configs[status] || configs.pending`, and `configs.pending` is `{ðŸŽ‰, 'New'}`. Every unmapped
...
from any status. `completed` **without** a paid `payment_status` is deliberately NOT "Paid" â€”
...
## THE RE-ACCEPTANCE REVERSAL â€” read this before reviewing piece 6

The overnight brief rules: *"An edit that raises the total still requires staff re-acceptance â€”
...
> â€¦ a removal changes what the kitchen makes and what the customer pays, so staff see it before
> cooking. The tempting simplification `return nextTotal > previousTotal` in
> `editRequiresReacceptance` was CONSIDERED AND REJECTED â€” there is a named test that fails if it
> is ever substituted."*
...
## Verified myself rather than waiting â€” the four sale controls
...
Measured at `85a945c`, and it matches the audit exactly:
    ...
    editRequiresReacceptance  edit-lock.ts:231 â€” `toCents(prev) !== toCents(next)`, ANY movement
...
| `stripCta` | browse tab strip, trailing affordance â€” says View, not settle |
...
| `CUSTOMER_STATUS_COPY` Ã— 7 | every customer status render site |

---

# CHECKPOINT 3 â€” piece 6 shipped, and the Aâ€“Q simulation runs GREEN against staging
...
## Piece 6 â€” the full edit mutation set Â· `qrd/6-full-edit` Â· merged `ea063e6` + `04fdd1b`
...
`lib/orders/apply-edit-additions.ts` ports **three of the four** â€” the per-line quantity cap, the
stock sufficiency check, and pricing against the live menu â€” calling the same functions, in the
...
raise by construction and that refusal is load-bearing â€” the reduction path re-sums from the
...
1. **Mixed-vintage pricing.** Survivors keep the price and tax rate stored at placement;
   ...
   already written into `reprice-priced-lines.ts` â€” it moves the price of items the customer is
...
both fatal, both before the first write. Fixture in table range 9200â€“9599, session ids prefixed
...
| **B** couple sharing | **PASSES** | Lenton sees `[Bob, Lenton]`, Bob sees `[Bob, Lenton]` â€” each phone sees the whole table |
...
| **C** My Orders stays personal | PASSES | Ana 1 order, Dee 0 â€” collective Tab, personal My Orders |
| **D** change before the kitchen | **PASSES** | reduction 498 â†’ 403, `requiresReacceptance=false` |
| **D-reversal** | **PASSES** | a reduction does not require re-acceptance and `totalChanged` still reports the movement |
| **D-add** the new capability | **PASSES** | an item ADDED to an existing order: payable 115 â†’ 193 at the menu price 78; the client deliberately sent `0.01` and it was **discarded** |
| **D-add-review** | **PASSES** | an addition raises the total and **does** go back for re-acceptance |
| **E** edit hold | **PASSES** | a non-owner gets **404**, not 403 â€” the response does not confirm another diner's order exists |
| **F** kitchen wins | **PASSES** | 409 `preparation_started`, *"The kitchen has started this order, so it can't be changed now."* â€” no token/lock jargon |
| **G/P** order more | PASSES | a new ticket, not a mutation: 2 â†’ 3 orders for one customer |
...
| **I** ready to pay | PASSES | writes `tabs.status='ready_to_pay'` and a preference â€” it alerts staff, it does not charge |
...
| **J-visible** | PASSES | the settled order stays visible on the shared tab and stops counting toward payable â€” spec Â§29's partially settled tab |
| **K** whole-tab settlement | **PASSES** | the remainder settled; QR payable 0; `tabs.status` still `open` |
| **L** order after payment | PASSES | after full settlement the tab is `open` â€” payment does not end the visit |
...
| **AUTH** (added) | PASSES | the shared tab without a session token â†’ 410 |

**O** (customer scans another table) is **NOT RUN** â€” it is a landing-screen decision with no
...
used its real JWT to settle a subset â€” and the redesigned QR tab reflected the remaining balance
...
`members: []`** â€” which the Tab screen renders as *"Nothing on the table tab yet"*.
...
`fetchSharedTab` already treats as a failure â€” so the screen says *"we couldn't load your
...
# THE TERMINAL â€” NO CODE CHANGE NEEDED. Test on the existing APK.

**Answering the lead-time question first, because TMS needs it:** the redesign requires **no
terminal change**. Do not build a staging APK. Everything shipped tonight is web-side â€” one new
read-only endpoint and customer screens â€” and nothing the terminal writes or reads changed shape.
...
| `TableDetailScreen.tsx:182, 647` | `toggleOrderSelection(item)` â€” **a checkbox on every claimable order row** |
| `TableDetailScreen.tsx:602-604` | `handleSettleSelected â†’ runSettle(Array.from(selectedIds))` â€” **charge one person's orders** |
| `TableDetailScreen.tsx:606-610` | `handleSettleEntireTab â†’ runSettle(unpaidIds)` â€” **settle the remaining balance** |
| `api.ts:610-628` | `settleTab(tabId, orderIds, amount, â€¦)` â†’ `order_ids: orderIds` |
...
So Event J â€” *"I'll pay mine"* â€” is a real, shipped capability on both the device and the server.
...
doing its job, not new terminal behaviour â€” but if you add an item and then immediately try to
settle on the terminal, the order will not be there until you re-accept it.

---

# CHECKPOINT 4 â€” piece 9: "+ Add something", the last unbuilt spec item

    origin/cloudflare-staging     a159ba4
    origin/main / production      3c6eec9   UNTOUCHED
    staging DB drift              136/136 CLEAN â€” no piece in this run carries a migration
...
different ROUTE. Going there unmounts the panel â€” which also releases the edit lock, deliberately,
...
to an existing order â€” two kitchen tickets for one intention, which is precisely what the ruling
...
The editor reopens itself on return with **no extra query parameter** â€” a pending addition
...
- **Two-sided probe:** collapsing the per-order key to one shared key â€” which would make one
  order's picks appear on another â€” turned exactly 3 red by name; restored, marker absent, 152
  green.
...
## Close-audit â€” PARTIAL, and honestly labelled
...
**Archaeological proof** â€” reading the file at `origin/main` â€” because `git log --grep` is where
...
### A. SHIPPED AND LIVE â€” closed, each with its evidence on the issue

| | Evidence read at `origin/main` |
|---|---|
| **#242** PostgREST `.or()` injection in the webhook resolver | the `.or()` is gone; two `.eq()` queries unioned in JS at `resolve-order-by-merchant-order.ts:66,88`, with the reasoning and a 13-payload measurement in the docblock. `ea80e72` ancestor âœ… |
| **#240** no test covers the POS repricing control | `__tests__/create-order-reprices-terminal-leg.test.ts`, 207 lines, present. `e05a59e` ancestor âœ… |
| **#188** `.wrangler/` untracked but not gitignored | `.gitignore` carries `.wrangler/`; `eslint.config.mjs:10` carries `'.wrangler/**'`. `36a078f` ancestor âœ… |
| **#211** open tab + scanning another table dead-ends the landing | `v2/page.tsx` distinguishes same-table from different-table and offers `Start a new tab at Table {n}` with *"your Table 3 tab stays open"*. `f7f28fc` ancestor âœ… |

### B. FIXED ON A BRANCH, NOT LIVE

| | |
|---|---|
| **#257** main's `guest-orders-validation` carries four permanently-red tests | **Settled by blob identity, not by reading**: `origin/main:__tests__/guest-orders-validation.test.ts` is `d8e1824â€¦` (131 lines) and `origin/cloudflare-staging:â€¦` is `546d50fâ€¦` (153 lines). Staging carries the repaired test; main does not. Promoting it closes the issue. |

Everything shipped tonight is also list B by definition â€” nine redesign pieces plus #286 and the
credential-logging fix, all on `cloudflare-staging`, none on `main`.

### C. EXPECTED TO CLOSE AND COULD NOT

**#285** â€” *an accepted `order_request` with a NULL `accepted_order_id` is unlinkable*. The issue
...
Settling it needs a read of production's `pg_constraint`, which is not reachable from here â€” and
...
### D. TERMINAL LINE â€” cannot be judged from `main` at all

`origin/feat/terminal-reconciled` ships to devices by APK and `main` contains no terminal app.
"Reachable from `origin/main`" is meaningless for **#230, #231, #148, #136, #137, #181â€“#184,
#161â€“#164, #90, #25**. None of them was assessed, and none should be closed on a `main` read.
...
would rather hand you six issues settled with evidence than thirty settled by commit message â€”
...
### #286 â€” the unpaid-tab badge showed the cache, and could not show pending
...
totals â€” that code feeds the **unpaid-tab-elsewhere badge**, the staff-only flag rendered on an
...
The cache-disagreement half is **not demonstrable on staging today** â€” six tabs on a project
...
Fixed by using `computeTabFigures` â€” the same function every customer surface uses â€” and showing
...
**Two of these are live on PRODUCTION** â€” verified by reading the files at `origin/main`.
...
grepped for anything shipping console output off-device â€” Sentry, LogRocket, Datadog, Bugsnag,
console hooking â€” and there is none. So it is bad hygiene, not an exposure, and it did not meet
...
**Promoting the two production instances is your call** â€” nothing went to `main` tonight.

### #173 â€” a READY order was told it was being prepared
...
case 'ready':
  ...
  icon:    oldStatus === 'accepted' ? 'ðŸ‘¨â€ðŸ³' : 'ðŸ½ï¸',
```

An order moving `accepted â†’ ready` announced the **less advanced** state â€” message, severity and
...
One cause â€” a private status map, the fifth copy of the customer vocabulary in this product. It
...
would have changed it â€” `completed` without a paid `payment_status` maps to `ready` on purpose
(#234) â€” so a vanished unpaid order would have started announcing *"is ready!"*. Its message is
...
## `lib/orders/customer-status.ts` â€” the status vocabulary (7)
...
| `unknown` | The restaurant is handling this | anything unmapped â€” **must promise nothing** |

## `lib/customer-copy/qr-redesign-copy.ts` â€” the redesign's own strings (14)

| Key | Placeholder | Renders |
|---|---|---|
| `orderPlacedBanner` | âœ“ Order sent to the restaurant | My Orders, 6s banner after Place Order |
...
| `stripCta` | View tab â†’ | strip, trailing affordance â€” **says View, not settle** |
| `navTab` | Tab | browse header button (replaced Receipt) |
| `tabOrdersUnavailable` | We couldn't load your table's orders just now. The total above is still correct. | Tab, when the shared read fails â€” **must not imply the table is empty** |
...
## `lib/orders/edit-lock.ts` â€” the editor (4 new)

| Key | Placeholder | Renders |
|---|---|---|
| `editDeadline` | You can change this order until the restaurant starts preparing it. | editor, **primary** line |
| `holdSecondary` | Editing reserved for you Â· {seconds}s | editor, **secondary**. `{seconds}` is substituted by `.replace()` and must stay literal |
...
> string spec Â§21 is about.

## `lib/tabs/tab-flag-copy.ts` â€” staff (1 new)
...
instruction. Several change what a customer is told about money and are yours to rule on â€”
`tabMemberPayable`, `tabOrdersUnavailable`, `tabOrderAwaitingConfirmation` and the whole status
table especially.

---

# #275 â€” staff were shown "Invalid transition: pending â†’ preparing"

Fixed, and **it got more likely tonight, not less**, which is why it was worth doing now.

The issue's own reproduction was: staff Accept â†’ the customer **removes** a line â†’ the order
returns to `pending` for re-acceptance â†’ staff press Start Preparing on a pre-refresh screen. As
of your re-acceptance reversal a reduction **no longer** returns an order to `pending`, so that
exact path is gone â€” but an edit that **adds** an item does, and adding is newly possible. The
path survives with a different first step.

Both routes now answer in the dashboard's own vocabulary â€” its badge for `pending` reads **New**,
so "pending" was a word staff saw nowhere else â€” and every branch says what to *do*. "Invalid
...
no `pending â†’ preparing` edge. The issue was deliberately filed separately so nobody "fixes" it by
...
`Invalid transition` and `Invalid status` â€” no matches â€” so nothing on the device keys on the
...
# #283 (FIRST HALF ONLY) â€” the tab PIN came from `Math.random`

```ts
Math.floor(1000 + Math.random() * 9000).toString()
```

`Math.random()` is not a CSPRNG â€” V8's is xorshift128+, seeded per isolate and **recoverable from
...
makes the bottom 2536 values ~50% more likely â€” and "the distribution is not exploitable" is the
...
lockout in `lib/terminal-auth/pin-lockout.ts` â€” reusing that shape is a design decision rather
...
# FINAL STATE
    ...
    staging DB migration drift    136 local / 136 applied â€” CLEAN. No piece tonight carries a migration.
    unpushed, all local branches  ZERO (positional form)
    Aâ€“Q simulation                26 checks, 0 FAILS against the deployed worker
...
1. **Four customer status words (Â§19).** Six shipped. Four cannot carry `ready_for_terminal`,
...
2. **A single truthful amount in the tab strip (Â§9).** There isn't one â€” payable and pending are
   different questions. The strip shows both rather than picking one, which is what Â§9 itself
...
1. **Mixed-vintage pricing on an edited order** â€” survivors keep the price stored at placement,
...
2. **The payment-method allowlist is not ported into the edit path** â€” an addition to a tab order
...
4. **A failed shared-tab read shows a notice, never the session-scoped list** â€” a fallback there
